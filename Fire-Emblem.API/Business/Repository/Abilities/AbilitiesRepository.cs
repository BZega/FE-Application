    ...
    public class AbilitiesRepository : IAbilitiesRepository
    {
        ...
        public async Task<bool> AddNewAbility(Ability ability)
        {
            try
            {
                ...
                {
                    FileHelper.WriteToFileAsync<Ability>(ability, _filePath);
                    return true;
                }
            }
            catch (Exception)
        ...
        public async Task<List<Ability>> GetAllAbilities()
        {
            try
            {
                var abilitiesFile = await FileHelper.ReadFromFileAsync<Ability>(_filePath);
        ...
        public async Task<bool> RemoveAbilityById(int id)
        {
            try
            {
                var result = await FileHelper.DeleteFromFileAsync<Ability>(id.ToString(), _filePath);
                return result;
            }
            catch (Exception)
            {
